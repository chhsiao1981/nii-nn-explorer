...
const metaExtractorToD3MetaCore = (
  meta: MetaExtractor,
  parent?: D3Node,
): D3Node => {
  const node: D3Node = {
  ...
  const preprocesses = meta.preprocesses.map((each, flowID) =>
    metaItemsToD3Meta(
      ...
      meta.name,
      flowID,
      TaskflowType_pb.T_PREPROCESS,
      node,
    ),
  );

  const nodes = meta.nodes.map((each) =>
    metaNNNodeToD3Meta(each, meta.name, node),
  );
  ...
  const extractors = meta.extractors.map((each) =>
    metaExtractorToD3MetaCore(each, node),
  );
...
export const metaTaskflowToD3Meta = (
  ...
  if (meta.the_type === TaskflowType.FORWARD) {
    const child = d3NNNodeToSeqD3NNNode(
      nodes[0],
      TaskflowType_pb.T_FORWARD,
      meta.flow_id,
    );
    node.children = [child];
    return node;
...
const d3ItemToSeqD3Item = (node: D3Node): D3Node => {
  return node;
};

/* eslint-disable */
const d3ItemsToSeqD3ItemsParseName = (node: D3Node, opType: OpType) => {
  /* eslint-enable */
  const nameList = [];
  if (node.name) {
...
const d3NNNodeContentToSeqD3NNNodeContent = (
  ...
  if (node.theType !== D3NodeType.MetaNNNodeContent) {
    console.warn(
      "d3NNNodeContentToSeqD3NNNodeContent: not expected type: node:",
      node,
    );
  }
  ...
  newNode.children = newNode.children.map((each) => {
    ...
    console.warn(
      "d3NNNodeContentToSeqD3NNNodeContent: invalid name:",
      newNode.name,
    );
    return each;
  });
...
export const metaItemToD3Meta = (
  ...
  const dataID =
    meta.the_type === ItemType.OP_ITEM ? meta.value.data_id : meta.data_id;
...
export const metaNNNodeToD3Meta = (
  ...
  if (meta.inputs?.length) {
    const inputs = meta.inputs.map((each) =>
  ...
  if (meta.params?.length) {
    const params = meta.params.map((each) =>
  ...
  if (meta.gradients?.length) {
    const gradients = meta.gradients.map((each) =>
      metaNNRecordToD3Node(each, extractorID, node),
    );
    node.children.push({
  ...
  if (meta.children?.length) {
    const children = meta.children.map((each) =>
      metaNNNodeToD3Meta(each, extractorID, node),
    );
    node.children.push({
...
const metaNNRecordToD3Node = (
  meta: MetaNNRecord,
  extractorID: string,
  parent?: D3Node,
): D3Node => {
  const dataID =
    meta.the_type === RecordType.OP_ITEM ? meta.record.data_id : meta.data_id;
  const node: D3Node = {
    name: meta.name,
...
const parseMetaNNRecordPostfix = (meta: MetaNNRecord): string => {
  ...
  return Object.hasOwn(meta.record, "shape")
    ? parseMetaNNTensorPostfix(meta.record as MetaNNTensor)
    : "";
};

const metaNNTensorToD3Node = (
  meta: MetaNNTensor,
  extractorID: string,
  parent?: D3Node,
): D3Node => {
...
const metaArrayNNTensorToD3Node = (
  ...
  const children = meta.map((each) =>
    metaNNTensorToD3Node(each, extractorID, node),
  );
